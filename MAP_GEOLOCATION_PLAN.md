...
```ts
{
  ...
  idleIntervalSec: number           // default 600
...
await Location.startLocationUpdatesAsync(LOCATION_TASK, {
  ...
  timeInterval: moving ? 60_000 : 600_000,  // приходит с сервера
...
(60 с в движении → 600 с в покое), а шум на месте режет `filters.ts` уже после
...
**Частоту задаёт сервер.** Оба интервала приходят в ответе на каждый батч
(`movingIntervalSec` / `idleIntervalSec`) и сохраняются в состоянии трекера,
поэтому подкрутить их можно настройками семьи, не выпуская новую сборку.
Значения в `config.ts` — лишь дефолт на первый запуск.

**Почему покой именно 10 минут.** Это наибольшее значение, которое ещё меньше
`STALE_AFTER_SEC` (15 минут): неподвижный телефон отчитывается достаточно часто,
чтобы его метка ни разу не стала серой. Плата — задержка обнаружения выхода:
ребёнок вышел из школы, а приложение заметит это в течение 10 минут и только
тогда перейдёт на минутный интервал. Убирается это не учащением опроса, а
геозоной вокруг точки покоя (P4): выход из неё ОС сообщает мгновенно и почти
бесплатно, и тогда интервал покоя можно поднять ещё.
