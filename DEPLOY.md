...
| `NOTIFICATIONS_RETENTION_DAYS` | сколько дней хранить уведомления (по умолчанию `30`, `0` — не чистить) |
| `COMPLETIONS_RETENTION_DAYS` | сколько дней хранить выполнения (по умолчанию `0` — **выключено**, см. ниже) |
...
Ночная задача в 4:00 чистит старые записи. Прогон вручную и проверка без
изменений:
...
**Уведомления** — 30 дней по умолчанию. Это расходный материал интерфейса: на
них никто не ссылается и ничего из них не считается.

**Выполнения** — по умолчанию **выключено**, включается только явной установкой
`COMPLETIONS_RETENTION_DAYS`. Это не уборка мусора: на выполнения ссылаются
записи баланса, по их `performedAt` считается «сколько баллов за день»,
условия бейджей считают их количество, а у родителя из них складывается
«выполнено заданий». Удаление меняет эти цифры безвозвратно — на проде окно в
30 дней убрало бы 3434 записи из 3971.
...
Сверяет каждую сохранённую ссылку на картинку с файлом на диске и обнуляет
только те, за которыми ничего нет, — интерфейс тогда покажет заглушку вместо
битой картинки. Печатает старые адреса перед очисткой, чтобы можно было
перезалить. Идемпотентно.
